...
export const EntityConfig = memo(() => {
  ...
  const entityConfig = useDiagramStore(
    useCallback(
      (state) => state.configs.find((config) => config.userId === user.id),
      [user.id],
    ),
  );
  ...
  return (
    <>
      ...
      <Portal>
        <CustomTheme
          color={entityConfig ? entityConfig.color : "#2190ff"}
          id={user.id}
        >
          {entityConfig && (
            <EntityConfigModal
              ...
              key={JSON.stringify(entityConfig)}
            />
          )}
        </CustomTheme>
      </Portal>
